...
/* THUNDERMAME HACKS */
	...
	if (data == 0x00||data==0xfe)
	{
		...
		samples->stop(8);samples->stop(9);samples->stop(10);
		samples->stop(11);samples->stop(12);samples->stop(13);
	}
	...
	if (data == 0x16)
		samples->start (9, data , 0);

	if (data == 0x17)
		samples->start (10, data , 0);

	if (data == 0x18 || data == 0x19)
		samples->start (11, data , 0);
...
MACHINE_RESET_MEMBER(toaplan1_state,vimana)
{
	...
	samples->set_volume ( 1, 0.60);
	samples->set_volume ( 2, 0.60);
	samples->set_volume ( 3, 0.60);
	samples->set_volume ( 4, 0.60);
	samples->set_volume ( 5, 0.60);
	samples->set_volume ( 6, 0.60);
	samples->set_volume ( 7, 0.60);
	samples->set_volume ( 8, 0.60);
	samples->set_volume ( 9, 0.60);
	samples->set_volume ( 10, 0.60);
	samples->set_volume ( 11, 0.60);
	samples->set_volume ( 12, 0.60);
	samples->set_volume ( 13, 0.60);
