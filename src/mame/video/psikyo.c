...
static void draw_sprites( running_machine &machine, bitmap_ind16 &bitmap, const rectangle &cliprect, int trans_pen )
{
	...
	memcpy(state->m_spritebuf2, state->m_spriteram, 0x2000);
...
{
	// rising edge
	//m_mainram[0x10c9]  = 0xBEEF; // offset 4326/4
	if (memhack >= 0)
	{
		memhack++;
		if (memhack >= 100)memhack = -1;
	}
	else
	{
		if (typee == 1) //gunbird
		{
			m_mainram[0x10ca] = ((~ioport("CONTROL0")->read() & 0xff) << 16) | (~ioport("CONTROL1")->read() & 0xff);
		}
		else if (typee == 2) //strikers  589C / 4 = 1627 + 1 = 1628
		{
			m_mainram[0x1627] = ((~ioport("CONTROL0")->read() & 0xff) << 16) | (~ioport("CONTROL1")->read() & 0xff);
		}
		else if (typee == 3) //tengai   95CE / 4 = 2573 + 1 = 2574
		{
			m_mainram[0x2573] = (~ioport("CONTROL0")->read() & 0xff);
			m_mainram[0x2574] = ((~ioport("CONTROL1")->read() & 0xff) << 16) | m_mainram[0x2574];
		}
		else if (typee == 4) //sengoku ace  b992 / 4 = 2E64 + 1 = 2E65
		{
			m_mainram[0x2E64] = (~ioport("CONTROL0")->read() & 0xff);
			m_mainram[0x2E65] = (~ioport("CONTROL1")->read() & 0xff) << 16;
		}
	}
	if (state)
	{
