...
#include "sound/samples.h" /* THUNDERMAME HACKS */
...
static void ese_fadeout(running_machine &machine)
{
	...
	if (counter1 >= 20)
	{
		...
		if (fadeout_ready == 1)
		{
			...
			samples->set_volume (0, sample_vol1);
		}
		if (sample_vol1 == 0)
		{
			samples->stop (0);
...
/* THUNDERMAME HACKS */
UINT32 toaplan2_state::screen_update_toaplan2s(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	screen_update_toaplan2(screen, bitmap, cliprect);
	ese_fadeout(screen.machine());
	return 0;
}
