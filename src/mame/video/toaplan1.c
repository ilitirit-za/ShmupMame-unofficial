...
static void ese_fadeout(running_machine &machine)
{
	...
	if (vcounter1 >= 17)
	{
		...
		if (vfadeout_ready == 1)
		{
			...
			samples->set_volume (0, vsample_vol1);
		}
		if (vsample_vol1 == 0)
		{
			samples->stop (0);
...
static void ese_fadeout2(running_machine &machine)
{
	samples_device *samples = machine.device<samples_device>("samples");
...
// it's really correct?
	...
	if (tsundere != 0)
	{
		...
		if (wait>=(92+start2))
		{
			samples->stop ( 0);
			samples->set_volume ( 0, 1.00);
			samples->start ( 0, 0x7, 0);
			_start=0;
			start2=1;
			wait=0;
		}
		if (start2 == 0 && tsundere == 1) ese_fadeout2(machine());
		
		if (tsundere == 2) ese_fadeout(machine());// Vimana
	}
	return 0;
}
