...
UINT32 cps_state::screen_update_cps1(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	if (mmatrixx == 1)
		...
		if (memhack2 < 0)
		{
			if ( ((~ioport("CONTROL0")->read()) & 0x20)) //player wants autosword
			{
				autosword++;
				if (autosword > 25)	//25 frames is fastest autosword
				{
					m_mainram[0x5841] = (((~ioport("CONTROL0")->read()) | 0x10) & 0x1F) | (m_mainram[0x5841] & 0xFFE0); //ffb07c
					autosword = 0;
				}
				else	//don't fire
				{
					m_mainram[0x5841] = (~ioport("CONTROL0")->read() & 0x0F) | (m_mainram[0x5841] & 0xFFE0); //ffb07c
				}
			}
			else		//normal player 1 behavior
			{
				m_mainram[0x5841] = (~ioport("CONTROL0")->read() & 0x1F) | (m_mainram[0x5841] & 0xFFE0); //ffb07c
				if (autosword < 25)autosword++;
			}
			
			if ( ((~ioport("CONTROL1")->read()) & 0x20)) //player2 wants autosword
			{
				autosword2++;
				if (autosword2 > 25)
				{
					m_mainram[0x5843] = (((~ioport("CONTROL1")->read()) | 0x10) & 0x1F) | (m_mainram[0x5842] & 0xFFE0); //ffb07c
					autosword2 = 0;
				}
				else	//don't fire
				{
					m_mainram[0x5843] = (~ioport("CONTROL1")->read() & 0x0F) | (m_mainram[0x5842] & 0xFFE0); //ffb07c
				}
			}
			else  //normal player 2 behavior
			{
				m_mainram[0x5843] = (~ioport("CONTROL1")->read() & 0x1F) | (m_mainram[0x5842] & 0xFFE0);
				if (autosword2 < 25)autosword2++;
			}
...
if (m_cps_version == 2)
	{
		...
		memcpy(m_cps2_buffered_obj, cps2_objbase(machine()), m_cps2_obj_size);
	}
if (m_cps_version == 1)
	{
