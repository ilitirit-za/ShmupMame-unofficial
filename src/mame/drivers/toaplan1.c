...
static WRITE16_HANDLER(samesame_mcu_w)
{
	...
	{
		samples->stop(0);samples->stop(1);samples->stop(2); //(samples, 2);
		samples->stop(3);samples->stop(4);samples->stop(5);
		samples->stop(6);samples->stop(7);samples->stop(8);
	}

	if (data >= 0x01 && data <= 0x05)
	{
		samples->set_volume(0,1.00); //(samples, 0,1.00);
		vfadeout_stop = 1;
		start2=0;
		samples->start(0,data,1); //(samples, 0, data , 1);
	}

	if (data == 0x06) {
		samples->start(0,data,0);
		start2=72;
		}
	...
	if (data == 0x08 || data == 0x09)
		samples->start(1,data,0);

	if (data >= 0x0a && data <= 0x0f)
		samples->start(2,data,0);

	if (data == 0x10 || data == 0x11)
		samples->start(3,data,0);

	if (data >= 0x12 && data <= 0x16)
		samples->start(4,data ,0);

	if (data >= 0x17 && data <= 0x1b)
		samples->start(5,data,0);

	if (data >= 0x1c && data <= 0x1f)
		samples->start(0,data,0);

	if (data == 0x20 || data == 0x21)
		samples->start(6,data,0);

	if (data == 0x22)
		samples->start(0,data,0);

	if (data == 0x23 || data == 0x24)
		samples->start (7,data,0);
	...
	if (data == 0x28)
		samples->start(8,data,0);
}

static const char *const samesame_sample_names[] =
{
	"*samesame",
...
static const samples_interface samesame_samples_interface =
{
...
static const samples_interface vimana_samples_interface =
{
...
//  AM_RANGE(0x100000, 0x100001) AM_WRITE_LEGACY(?? video frame related ??)
    ...
    AM_RANGE(0x14000e, 0x14000f) AM_WRITE_LEGACY(samesame_mcu_w)   /* Commands sent to HD647180 - THUNDERMAME HACKS */
...
static MACHINE_CONFIG_START( samesame, toaplan1_state )
	...
	MCFG_SCREEN_ADD("screen", RASTER)
	MCFG_SCREEN_RAW_PARAMS(PIXEL_CLOCK, HTOTAL, HBEND, HBSTART, VTOTAL, VBEND, VBSTART)
	//MCFG_SCREEN_UPDATE_DRIVER(toaplan1_state, screen_update_samesame) // THUNDERMAME HACKS
	MCFG_SCREEN_UPDATE_DRIVER(toaplan1_state, screen_update_toaplan1)
	...
	/* THUNDERMAME HACKS */
	MCFG_SAMPLES_ADD("samples", samesame_samples_interface)
	//MCFG_SOUND_ADD("samples", SAMPLES, 0)
	//MCFG_SOUND_CONFIG(samesame_samples_interface)
	MCFG_SOUND_ROUTE(ALL_OUTPUTS, "mono", 1.0)	
MACHINE_CONFIG_END
...
static MACHINE_CONFIG_START( vimana, toaplan1_state )
	...
	/* THUNDERMAME HACKS */
	MCFG_SAMPLES_ADD("samples", vimana_samples_interface)
	//MCFG_SOUND_ADD("samples", SAMPLES, 0)
	//MCFG_SOUND_CONFIG(vimana_samples_interface)
	MCFG_SOUND_ROUTE(ALL_OUTPUTS, "mono", 1.0)
MACHINE_CONFIG_END
...
DRIVER_INIT_MEMBER(toaplan1_state,zerowing)
{
	toaplan1_driver_savestate(machine());
	//UINT16 *rom = (UINT16 *)machine.root_device().memregion("maincpu")->base();
	//rom[0xC7] = 0x4e71; //nop C7
	*(UINT16 *)(memregion("maincpu")->base() + 0x4DA) = 0x4e71; //hack rom check
	*(UINT16 *)(memregion("maincpu")->base() + 0x4DC) = 0x4e71;
	
	*(UINT16 *)(memregion("maincpu")->base() + 0x186) = 0x4e71; //then hack the game
	*(UINT16 *)(memregion("maincpu")->base() + 0x188) = 0x4e71;
	*(UINT16 *)(memregion("maincpu")->base() + 0x18A) = 0x4e71;
	*(UINT16 *)(memregion("maincpu")->base() + 0x18C) = 0x4e71;
}

DRIVER_INIT_MEMBER(toaplan1_state,zerowing2)
{
	toaplan1_driver_savestate(machine());
	//UINT16 *rom = (UINT16 *)machine.root_device().memregion("maincpu")->base();
	//rom[0xC7] = 0x4e71; //nop C7
	*(UINT16 *)(memregion("maincpu")->base() + 0x568) = 0x4e71; //hack rom check
	*(UINT16 *)(memregion("maincpu")->base() + 0x56A) = 0x4e71;
...
GAME( 1989, zerowing,   0,        zerowing, zerowing, toaplan1_state,  zerowing, ROT0,   "Toaplan", "Zero Wing (1P set)", 0 )
GAME( 1989, zerowing2,  zerowing, zerowing, zerowing2, toaplan1_state, zerowing2, ROT0,   "Toaplan", "Zero Wing (2P set)", 0 )
...
GAME( 1990, fireshrk,   0,        samesame, fireshrk, toaplan1_state,  fireshrk, ROT270, "Toaplan", "Fire Shark", 0 )
GAME( 1990, fireshrkd,  fireshrk, samesame, samesame2, toaplan1_state, fireshrk, ROT270, "Toaplan (Dooyong license)", "Fire Shark (Korea, set 1, easier)", 0 )
GAME( 1990, fireshrkdh, fireshrk, samesame, samesame2, toaplan1_state, fireshrk, ROT270, "Toaplan (Dooyong license)", "Fire Shark (Korea, set 2, harder)", 0 )
GAME( 1989, samesame,   fireshrk, samesame, samesame, toaplan1_state,  fireshrk, ROT270, "Toaplan", "Same! Same! Same! (1P set)", 0 )
GAME( 1989, samesame2,  fireshrk, samesame, samesame2, toaplan1_state, fireshrk, ROT270, "Toaplan", "Same! Same! Same! (2P set)", 0 )
GAME( 1990, outzone,    0,        outzone,  outzone, toaplan1_state,   toaplan1, ROT270, "Toaplan", "Out Zone (set 1) (broken rank version)", 0 ) // later fixed version
...
GAME( 1991, vimana,     0,        vimana,   vimana, toaplan1_state,    vimana,   ROT270, "Toaplan", "Vimana (World, set 1)", 0 )
GAME( 1991, vimanan,    vimana,   vimana,   vimanan, toaplan1_state,   vimana,   ROT270, "Toaplan", "Vimana (World, set 2)", 0 )
GAME( 1991, vimanaj,    vimana,   vimana,   vimanaj, toaplan1_state,   vimana,   ROT270, "Toaplan", "Vimana (Japan)", 0 )
