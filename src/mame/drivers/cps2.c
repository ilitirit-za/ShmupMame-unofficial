...
TIMER_DEVICE_CALLBACK_MEMBER(cps_state::cps2_interrupt)
{
	if (mmatrixx == 1)
	{
		...
		{
			memhack2++;
			if (memhack2 >= 200)memhack2 = -1;
		}
		else
		{
			m_mainram[0x5841] = (~ioport("CONTROL0")->read() & 0x0F) | (m_mainram[0x5841] & 0xFFF0); //ffb07c
			m_mainram[0x5843] = (~ioport("CONTROL1")->read() & 0x0F) | (m_mainram[0x5842] & 0xFFF0);
		}
	}
...
DRIVER_INIT_MEMBER(cps_state,mmatri)
{
...
GAME( 2000, mmatrix,    0,        cps2, cps2_2p2b, cps_state, mmatri,     ROT0,   "Takumi (Capcom license)", "Mars Matrix: Hyper Solid Shooting (USA 000412)", GAME_SUPPORTS_SAVE )
GAME( 2000, mmatrixj,   mmatrix,  cps2, cps2_2p2b, cps_state, mmatri,     ROT0,   "Takumi (Capcom license)", "Mars Matrix: Hyper Solid Shooting (Japan 000412)", GAME_SUPPORTS_SAVE )
