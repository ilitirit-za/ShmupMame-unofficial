...
WRITE16_MEMBER(toaplan2_state::tekipaki_hd647180_cpu_w)
{
	...
	if (data == 0xfe)
		{
		samples->stop(0);samples->stop(1);samples->stop(2);samples->stop(3);
		samples->stop(4);samples->stop(5);samples->stop(6);samples->stop(7);
		}
	
	if (data >= 0x01 && data <= 0x03)
		samples->start (0, data, 1);
...
static const samples_interface tekipaki_samples_interface =
{
...
static const samples_interface ghox_samples_interface =
{
	9,ghox_sample_names
};
/* ----------- */
...
static ADDRESS_MAP_START( tekipaki_68k_mem, AS_PROGRAM, 16, toaplan2_state )
	...
	AM_RANGE(0x180070, 0x180071) AM_WRITE(tekipaki_hd647180_cpu_w) // THUNDERMAME HACKS
ADDRESS_MAP_END
...
static MACHINE_CONFIG_START( tekipaki, toaplan2_state )
	...
	MCFG_SCREEN_UPDATE_DRIVER(toaplan2_state, screen_update_toaplan2s) /* THUNDERMAME HACKS */
	...
	MCFG_SAMPLES_ADD("samples", tekipaki_samples_interface) /* THUNDERMAME HACKS */
...
static MACHINE_CONFIG_START( ghox, toaplan2_state )
	...
	MCFG_SAMPLES_ADD("samples", ghox_samples_interface) /* THUNDERMAME HACKS */
	MCFG_SOUND_ROUTE(ALL_OUTPUTS, "mono", 1.0)
MACHINE_CONFIG_END
...
GAME( 1991, tekipaki,   0,        tekipaki, tekipaki, driver_device, 0,        ROT0,   "Toaplan", "Teki Paki", GAME_SUPPORTS_SAVE )

GAME( 1991, ghox,       0,        ghox,     ghox, driver_device,     0,        ROT270, "Toaplan", "Ghox (spinner)", GAME_SUPPORTS_SAVE )
GAME( 1991, ghoxj,      ghox,     ghox,     ghox, driver_device,     0,        ROT270, "Toaplan", "Ghox (joystick)", GAME_SUPPORTS_SAVE )
