...
void ui_menu_autofire::handle()
{
	...
	if (menu_event != NULL && menu_event->itemref != NULL)
	{
		if (menu_event->iptkey == IPT_UI_LEFT || menu_event->iptkey == IPT_UI_RIGHT)
		{
			...
			//autofire delay
			if (player >= 100 && player < (MAX_PLAYERS + 100))
			{
				int autofire_cust = machine().ioport().get_autofirecustom(player - 100);
				...
				machine().ioport().set_autofirecustom(player -100, autofire_cust);

				changed = true;
			}
			else if (player >= 0 && player < MAX_PLAYERS)
			{
...
//				entry[menu_items] = field;
	...
	for (i = 0; i < players; i++)
	{
		...
		subtext.printf("%d = %.2fHz", machine().ioport().get_autofiredelay(i), (float)60/(machine().ioport().get_autofiredelay(i)+1));
	...
	for (i = 0; i < players; i++)
	{
		text.printf(_("P%d %s"), i + 1, _("Custom Btn Autofire"));
		subtext.printf("%d = %.2fHz", machine().ioport().get_autofirecustom(i), (float)60/(machine().ioport().get_autofirecustom(i)+1));

		// append a menu item
		item_append(text, subtext, MENU_FLAG_LEFT_ARROW | MENU_FLAG_RIGHT_ARROW, (void *)(i + AUTOFIRE_ITEM_P1_DELAY + 100));  //+100 triggers custom
	}
}
